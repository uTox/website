...
gulp.task('default', [
  'templates',
  'styles',
  // 'scripts', // Javascript isn't used in uTox.org at this time
  'images',
  'etc'
]);
...
gulp.task('serve', () => {
  // Rebuild templates on change in src/
  gulp.watch(['src/*.njk', 'src/templates/**/*'], ['templates']);
  ...
  gulp.watch(['src/**/*', '!src/{template,styles,scripts,images}/**/*'], ['etc']);
...
// `gulp templates` - Build HTML from Nunjucks source
gulp.task('templates', () =>
  gulp.src(['src/**/*.njk', '!src/{templates,templates/**/*}'])
    // Generate HTML from Nunjuck templates
    .pipe(g.nunjucks.compile())
    .pipe(g.rename({
      extname: ".html"
    }))
    // Write generated HTML code to dist/
    .pipe(gulp.dest('dist/'))
    // Reload the browser
...
// `gulp scripts` - Build browser JS code using webpack (v1)
